 ...
 * Description:Allocate and return an array of sorted elements in the set pointed to by sp
 * allocates an array and sends to quicksort function to be sorted
 *
 *
 */
...
void *getElements(SET *sp)
{
	...
	quicksort(new,0,sp->count-1,sp->compare);//use quicksort to sort the newly allocated array
	return new;

}

//recursive functon used to sort elements in the array pointed to by elts
//calls partition to perform sorting on each itteration
//time complexity average: O(nlogn)
//time complexity worst: O(n^2)
static void quicksort(void**elts,int lo,int hi,int (*compare)())
{
	if(lo<hi)
	{
		int pi=partition(elts,lo,hi);
		quicksort(elts,lo,pi-1,compare); //left side
		quicksort(elts,pi+1,hi,compare);//right side
	}
	return;
}
//returns index of the partition lovation
//sorts array given such that left of partition is smaller than partition value and right of partition is larger than partition value
//time complexity: O(n)
static int partition(void **elts,int lo, int hi, int (*compare)())
{
	void *pivot=elts[hi];//select rightmost element as the pivot
	void *temp;
	int pindex=lo;
	int i;
	for(i=lo;i<hi;i++)
